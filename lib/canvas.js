...
Canvas.prototype = {
  ...
  'export': function (options) {
    ...
    applyPixelExtruding(img, xOffset, yOffset, ndarray, 4);
...
function applyPixelExtruding(img, xOffset, yOffset, ndarray, size) {
  ...
  for (let colIndex = 0; colIndex < img.width; colIndex += 1) {
    for (let rgbaIndex = 0; rgbaIndex < rgbaCount; rgbaIndex += 1) {
      ...
      for (let i = 0; i < size; ++i) {
        ndarray.set(xOffset + colIndex, yOffset - 1, rgbaIndex, val);
      }
    }
  }
  // bottom
  for (let colIndex = 0; colIndex < img.width; colIndex += 1) {
    for (let rgbaIndex = 0; rgbaIndex < rgbaCount; rgbaIndex += 1) {
      ...
      for (let i = 0; i < size; ++i) {
        ndarray.set(xOffset + colIndex, yOffset + img.height, rgbaIndex, val);
      }
    }
  }
  // left
  for (let rowIndex = 0; rowIndex < img.height; rowIndex += 1) {
    for (let rgbaIndex = 0; rgbaIndex < rgbaCount; rgbaIndex += 1) {
      ...
      for (let i = 0; i < size; ++i) {
        ndarray.set(xOffset - 1, yOffset + rowIndex, rgbaIndex, val);
      }
    }
  }
  // right
  for (let rowIndex = 0; rowIndex < img.height; rowIndex += 1) {
    for (let rgbaIndex = 0; rgbaIndex < rgbaCount; rgbaIndex += 1) {
      ...
      for (let i = 0; i < size; ++i) {
        ndarray.set(xOffset + img.width, yOffset + rowIndex, rgbaIndex, val);
      }
    }
  }
}
